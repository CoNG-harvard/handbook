...
The inspected `~/robocoop/` folder is a workspace, not a top-level Git repository. Its inference code uses `--config` and a YAML settings file. The associated recording workspace is `~/lerobot/`.

Provide a dated source bundle containing:

| Folder/file | Why the new installation needs it |
|---|---|
| `lerobot/lerobot/` | Modified LeRobot source, including local recording and dataset changes |
| `lerobot/lerobot_robot_xarm/` | xArm driver plugin |
| `lerobot/lerobot_camera_imageclient/` | Image-server camera plugin |
| `lerobot/lerobot_teleoperator_xr/` | Quest control plugin |
| `lerobot/xr_teleoperate/teleop/teleimager/` and `televuer/` | Modified camera and headset packages, including camera configuration |
| `lerobot/scripts/` | Launcher, rig manifests, reset routine, and dataset preparation tools |
| `robocoop/run_xarm_inference.py` and `.yaml` | Inference client and complete settings |
| `robocoop/models/download_model.py` | Checkpoint downloader |
| `robocoop/openpi/`, with lockfile and submodules | Lab model-server fork and client package |
...
If the new workstation has authorized access to the reference machine, the maintainer can use `rsync` to copy selected directories into a **new, empty** local workspace. Replace `<ARM_SOURCE_HOST>` with the SSH alias supplied by the lab. Run on the **new workstation**:
...
rsync -av --exclude='.git' --exclude='.venv' --exclude='__pycache__' \
  ...
  '<ARM_SOURCE_HOST>:~/lerobot/lerobot' \
  '<ARM_SOURCE_HOST>:~/lerobot/lerobot_robot_xarm' \
  '<ARM_SOURCE_HOST>:~/lerobot/lerobot_camera_imageclient' \
  '<ARM_SOURCE_HOST>:~/lerobot/lerobot_teleoperator_xr' \
  '<ARM_SOURCE_HOST>:~/lerobot/xr_teleoperate' \
  '<ARM_SOURCE_HOST>:~/lerobot/scripts' ~/lerobot/
```
...
The active workstation checkout is `~/unidog_nav`, with origin [CoNG-harvard/unidog_nav](https://github.com/CoNG-harvard/unidog_nav), at commit `edee63b` plus local changes at review time. An older nested checkout exists inside `LLM_guided_RL`; do not use it as the current workstation source by accident.
...
The all-services helper `scripts/prepare_real_robot.sh` additionally expects:

- Workstation `~/LLM_guided_RL/scripts/serve_segmentation.py` and its dependencies.
- Workstation `~/.venvs/segserve/bin/python`, model assets, and a working segmentation service on port 8090.
